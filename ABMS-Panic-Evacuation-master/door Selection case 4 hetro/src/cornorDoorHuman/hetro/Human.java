...
import src.cornorDoorHuman.hetro.Crowd;
import src.cornorDoorHuman.hetro.Human;
import src.cornorDoorHuman.hetro.Vector2D;
import sim.util.Bag;
...
public class Human implements Steppable {
	...
	public Vector2D nearBuddy(Crowd c)
	{
		Vector2D mean= new Vector2D(0,0);
		int n=0;
		Bag bud=c.buddies.getEdges(this, null);
		//double d=0.0;
		int len = bud.size();

		for(int buddy = 0 ; buddy < len; buddy++)
		{
			Edge e = (Edge)(bud.get(buddy));
			double buddiness = ((Double)(e.info)).doubleValue();
			Human h= (Human)e.getOtherNode(this);

			Double2D him = new Double2D(h.x,h.y);
			if(buddiness>=0.7)
			{
				mean= mean.add(new Double2D((him.x - this.x) * buddiness, (him.y - this.y) * buddiness));
				
				//d=d+ Math.sqrt(distanceSquared(him.x,him.y,this.x,this.y));
				n++;
				isBuddy=true;
			}
	...
	public double distanceFromBuddy(Crowd c)
	{
		int n=0;
		int f=0;
		Bag bud=c.buddies.getEdges(this, null);
		...
		for(int buddy = 0 ; buddy < len; buddy++)
		{
			...
			if(buddiness>=0.7)
			{
				d=Math.sqrt(distanceSquared(him.x,him.y,this.x,this.y));
				if(d>this.buds*avoiDistance)
					f=1;
				n++;
			}
	...
	public void step(final SimState state) 
	{
		...
		{	//faster way to access the agent
			...
			this.UpperVisibility =(int)5*this.diameter;
		...
		if(d==-1)
		{
			...
			vel=vel.subtract(nearBuddy(bd).amplify(9.5));
			...
			vel = vel.add( getGoal(bd).amplify(6.5));

			//if you are near the exit and you can see so more eager to go to the goal
			if ( ( (Math.abs(HumanPosition.x -Crowd.doorLeft) <30) ||(Math.abs(HumanPosition.x -Crowd.doorRight)<30 ) ) 
		...
		{
			vel= vel.add(nearBuddy(bd).amplify(9.5));
			//if you are inside the box get the goal amplified by 5.5 otherwise 7.5
			if ( (HumanPosition.y> Crowd.yMinB ) || HumanPosition.y<Crowd.yMaxB);
			vel = vel.add( getGoal(bd).amplify(6.5));
